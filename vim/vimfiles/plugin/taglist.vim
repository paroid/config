...
function! s:Tlist_Window_Init()
    ...
    " this function is created by paroid for quick reset room taglist
    " call jump first because resize will mess up cursor position 
    " after jump set the cursor back to taglist window,get everything done &
    " go back
    function! MyTagJumpWithRoom()
        call <SID>Tlist_Window_Jump_To_Tag('useopen')
        :wincmd p
        if winheight(0) == &lines - 4 
            :res -11    
            call <SID>Tlist_Window_Zoom()
        endif 
        ...
        :wincmd p
    endfunc
    " map <CR> to MyTagJumpWithRoom()"
    nnoremap <buffer> <silent> <CR> :call MyTagJumpWithRoom()<CR>
